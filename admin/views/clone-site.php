<div class="wrap">
	<h2><?php echo esc_html( __( 'Webseite klonen', WPMUDEV_CLONER_LANG_DOMAIN ) ); ?></h2>

		<?php settings_errors( 'cloner' ); ?>
		<?php if ( ! empty( $blog_details ) ): ?>
			<p id="cloner-blog-details">
				<span class="cloner-head"><?php _e( 'Klonen', WPMUDEV_CLONER_LANG_DOMAIN ); ?></span> 
				...
				<span class="cloner-back-link"> <a href="<?php echo network_admin_url( 'sites.php' ); ?>"><?php _e( 'Wähle eine andere Webseite', WPMUDEV_CLONER_LANG_DOMAIN ); ?></a></span>
			</p>
		<?php endif; ?>
		<hr/>
		...
		<form method="post" action="<?php echo $form_url; ?>">
			...
			<?php submit_button( __( 'Webseite klonen', WPMUDEV_CLONER_LANG_DOMAIN ), 'primary', 'clone-site-submit' ); ?>
		</form>
</div>
...
<script>
	...
	function cloner_load_multiselector() {
		...
		jQuery('#additional-tables-selector').multiSelect({
			'selectableHeader': '<p class="multiselect-header"><?php _e( "Ignoriere diese Tabellen", WPMUDEV_CLONER_LANG_DOMAIN ); ?></p>',
			'selectionHeader': '<p class="multiselect-header"><?php _e( "Kopiere diese Tabellen", WPMUDEV_CLONER_LANG_DOMAIN ); ?></p>',
