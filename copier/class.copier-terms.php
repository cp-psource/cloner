...
if ( ! class_exists( 'Site_Copier_Terms' ) ) {
	class Site_Copier_Terms extends Site_Copier {
		...
		function wp_insert_term_wp_43( $term, $taxonomy, $args = array() ) {
			...
			if ( ! taxonomy_exists($taxonomy) ) {
				return new WP_Error('invalid_taxonomy', __('Ungültige Taxonomie'));
			}
			/**
			...
			if ( is_int($term) && 0 == $term ) {
				return new WP_Error('invalid_term_id', __('Ungültige Begriff-ID'));
			}
			if ( '' == trim($term) ) {
				return new WP_Error('empty_term_name', __('Für diesen Begriff ist ein Name erforderlich'));
			}
			$defaults = array( 'alias_of' => '', 'description' => '', 'parent' => 0, 'slug' => '');
			$args = wp_parse_args( $args, $defaults );

			if ( $args['parent'] > 0 && ! term_exists( (int) $args['parent'] ) ) {
				return new WP_Error( 'missing_parent', __( 'Übergeordneter Begriff existiert nicht.' ) );
			}
			$args['name'] = $term;
			...
			if ( $name_match = get_term_by( 'name', $name, $taxonomy ) ) {
				...
				if ( ! $slug_provided || $name_match->slug === $slug || $slug_match ) {
					if ( is_taxonomy_hierarchical( $taxonomy ) ) {
						...
						if ( $existing_term ) {
							return new WP_Error( 'term_exists', __( 'Bei diesem übergeordneten Element ist bereits ein Begriff mit dem angegebenen Namen vorhanden.' ), $existing_term->term_id );
						}
					} else {
						return new WP_Error( 'term_exists', __( 'In dieser Taxonomie existiert bereits ein Begriff mit dem angegebenen Namen.' ), $name_match->term_id );
					}
				}
			}
			...
			if ( false === $result ) {
				return new WP_Error( 'db_insert_error', __( 'Begriff konnte nicht in die Datenbank eingefügt werden' ), $wpdb->last_error );
			}
			...
			if ( $term_id = term_exists($slug) ) {
				...
				if ( is_taxonomy_hierarchical($taxonomy) && $existing_term['name'] == $name && $exists = term_exists( (int) $term_id, $taxonomy ) ) {
					...
					if ( in_array($name, $siblings) ) {
						if ( $slug_provided ) {
							return new WP_Error( 'term_exists', __( 'Bei diesem übergeordneten Element ist bereits ein Begriff mit dem angegebenen Namen und dem angegebenen Slug vorhanden.' ), $exists['term_id'] );
						} else {
							return new WP_Error( 'term_exists', __( 'Bei diesem übergeordneten Element ist bereits ein Begriff mit dem angegebenen Namen vorhanden.' ), $exists['term_id'] );
						}
					} else {
						$slug = wp_unique_term_slug($slug, (object) $args);
					...
					// Same name, same slug.
					return new WP_Error( 'term_exists', __( 'Ein Begriff mit dem angegebenen Namen und der angegebenen Schnecke ist bereits vorhanden.' ), $exists['term_id'] );
				}
			} else {
